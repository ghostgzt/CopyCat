 ...
 if($time-filemtime($fileforbid)>60){
  ...
  if($ip==substr($fileforbidarr[0],0,strlen($ip))) {
   if($time-substr($fileforbidarr[1],0,strlen($time))>600){
    ...
    file_put_contents($fileht,PHP_EOL."Deny from ".$ip,FILE_APPEND);
    unlink($fileforbid);
   } else { 
    $fileforbidarr[2]++;
...
//防刷新
$str="";
$file="_log/ipdate.dat";
if(!file_exists("_log")&&!is_dir("_log"))
 mkdir("_log",0777);
if(!file_exists($file))
 ...
 if($time-$timetem<$allowTime) {
  if($iptem!=$checkip)
   ...
   elseif($numtem<$allowNum)
    ...
    file_put_contents("_log/forbided_ip.log",$ip."--".date("Y-m-d H:i:s",time())."--".$uri."\r\n",FILE_APPEND);
