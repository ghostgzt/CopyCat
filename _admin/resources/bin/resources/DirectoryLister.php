    ...
    public function listBreadcrumbs($directory = null) {
        ...
        foreach ($dirArray as $key => $dir) {
            ...
            if ($dir != '.') {
                ...
                $link = $this->_appURL . /*'?dir=' . rawurlencode*/(PATH_SEPARATOR == ':')?rawurlencode($dirPath):rawurlencode(encode_iconv("gbk", "UTF-8", $dirPath));
                ...
                if ($file == '..') {
                    ...
                    if ($this->_directory != '.') {
                        ...
                        if (!empty($directoryPath)) {
                            $directoryPath = /*'?dir=' . rawurlencode*/(PATH_SEPARATOR == ':')?rawurlencode($directoryPath):rawurlencode(encode_iconv("gbk", "UTF-8", $directoryPath));
                        }
                    ...
                    if ($this->_directory != '.' || $file != 'index.php') {
                        ...
                        if (is_dir($relativePath)) {
                            $filePath = /*'?dir=' . rawurlencode*/(PATH_SEPARATOR == ':')?rawurlencode($relativePath):rawurlencode(encode_iconv("gbk", "UTF-8", $relativePath));
                        } else {
    ...
    protected function _isHidden($filePath) {
        ...
        foreach ($this->_config['hidden_files'] as $hiddenPath) {
			if(!function_exists("fnmatch")){
				if (@preg_match($hiddenPath, $filePath)) {
